function ActivityBar(articleId, initialLikes) {
    ...
    updateModerationStatus = function(text) {
        if(document.querySelector(".layout-card.moderation-tools")) {
            document.querySelector(".layout-card.moderation-tools .status-label").innerText = text;
        } else {
    ...
    _this.buildModElement = function() {
        if (_this.canEdit || _this.canDelete) {
            ...
            if (_this.canDelete) {
                dom += '<li class="dropdown-item"><a class="mod-delete-option" href="#">Delete</a></li>'
            }
            ...
            _this.modDeleteOption = document.querySelectorAll('.mod-delete-option')
    ...
    _this.bindOptionToggles = function() {
        if (_this.modDeleteOption) {
            for(var i = 0; i < _this.modDeleteOption.length ; i++) {
                _this.modDeleteOption[i].addEventListener('click', function() {
